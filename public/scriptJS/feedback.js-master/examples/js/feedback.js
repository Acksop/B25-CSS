...
window.Feedback = function( options ) {
    ...
    // default properties
    options.ip = options.ip || "anonymous";
    options.label = options.label || "Signalement";
    options.header = options.header || "Validation de votre signalement";
    //options.url = options.url || "https://xn--besanon25-u3a.fr/index.php?traitement=traitementSignalementCourriel";
    //options.adapter = options.adapter || new window.Feedback.REDIRECT( options.url, options.ip );
    options.url = options.url || "https://xn--besanon25-u3a.fr/index.php?traitementAJAX=postCourrielSignalement";
    options.adapter = options.adapter || new window.Feedback.XHR( options.url, options.ip );
    options.nextLabel = options.nextLabel || "Continuer";
    options.reviewLabel = options.reviewLabel || "Vérifier";
    options.sendLabel = options.sendLabel || "Envoyer";
    options.closeLabel = options.closeLabel || "Fermer";
    
    options.messageSuccess = options.messageSuccess || "Votre signalement à été enregistré !";
    options.messageError = options.messageError || "Oups, quelque-chose s'est mal passé ! Pouvez vous recommencer ?";
...
window.Feedback.Form = function( elements ) {
    ...
    this.elements = elements || [{
        type: "textarea",
        name: "Issue",
        label: "Pouvez vous décrire le pourquoi de votre signalement ?",
        required: false
    }];
...
window.Feedback.Screenshot.prototype.start = function( modal, modalHeader, modalFooter, nextButton ) {
    ...
    if ( this.h2cDone ) {
        ...
        blackoutButton = element("a", "Cacher une zone"),
        highlightButton = element("a", "Mettre en évidence une zone"),
        previousElement;
        ...
        this.dom.appendChild( element("p", "Selectionner les zones que voulez mettre en évidence") );
...
window.Feedback.XHR = function( url, ip ) {
...
window.Feedback.XHR.prototype.send = function( data, callback ) {
    ...
    compressImage(data[1]).then(function(result) {

        //console.log(result);

        datas = [data[0], ip, result];

        //console.log(datas);

        //datas = [data[0], this.ip,  data[1]];
        ...
        var params = "data=" + encodeURIComponent(window.JSON.stringify(datas));
...
window.Feedback.REDIRECT = function( url , ip) {
    this.url = url;
    this.ip = ip;
};

window.Feedback.REDIRECT.prototype = new window.Feedback.Send();

window.Feedback.REDIRECT.prototype.send = function( data, callback ) {
    ...
    //compressImage(data[1]).then(function(result) {
        ...
        //datas = [data[0],ip, result];
        datas = [data[0],ip, data[1]];

        var form = $('<form action="' + url + '" method="post">' +
            '<input type="text" name="data" value="' + encodeURIComponent(window.JSON.stringify(datas)) + '" />' +
            '</form>');
        $('body').append(form);
        form.submit();
    //});
    ...
    //window.location.replace(this.url + "?data="+encodeURIComponent( window.JSON.stringify( data )));
